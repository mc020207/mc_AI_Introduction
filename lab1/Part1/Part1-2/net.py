...
class MultiCrossEntropyLoss():
    ...
    def backward(self):
        loss_grad_predicts=-(self.labels/self.predicts-(1-self.labels)/(1-self.predicts))/self.num
        self.model.backward(loss_grad_predicts)

class Linear():
    def __init__(self,input_size,output_size,name):
        self.params={}
        self.params['W']=(np.random.rand(input_size,output_size)*2-1)/np.sqrt(input_size)
        self.params['b']=-np.random.rand(1,output_size)
        self.inputs=None
        self.grads={}
        self.name=name

    def __call__(self,imputs):
        return self.forward(imputs)    
...
class softmaxLinear():
    def __init__(self,input_size,output_size,name):
        self.params={}
        self.params['W']=(np.random.rand(input_size,output_size)*2-1)/np.sqrt(input_size)
        self.params['b']=np.random.rand(1,output_size)*0.4-0.2
    ...
    def backward(self,labels):
        ...
        self.grads['b']=-1/N*np.matmul(np.ones([1,N],dtype=np.float64),(labels-self.outputs))
...
class Model_MLP_L2():
    def __init__(self,input_size,hidden_size1,hidden_size2,hidden_size3,output_size):
        ...
        self.linearsoft=softmaxLinear(hidden_size3,output_size,name="softmaxLinear")
        self.layers=[self.fc1,self.act_fn1,self.fc2,self.act_fn2,self.fc3,self.act_fn3,self.linearsoft]
    ...
    def load_model(self,model_dir):
        ...
        for layer in self.model.layers:
            if isinstance(layer.params,dict):
                ...
                layer.params=temp.item()

def accuracy(preds,labels,draw=False):
    preds=np.argmax(preds,axis=1)
    temp=[(preds[i]==labels[i])/1 for i in range(len(preds))]
    ans=np.mean(temp)
    if draw:
        for idx in range(len(temp)):
            if (temp[idx]!=1):
                print(idx,preds[idx],labels[idx],end=' | ')
    return ans

class MLPClassifier(object):
    def __init__(self,input_dim,output_dim):
        self.model=Model_MLP_L2(input_size=input_dim,output_size=output_dim,hidden_size1=1024,hidden_size2=2048,hidden_size3=512)
        self.runner=None
    ...
    def fit(self,X_train,y_train,X_validate,y_validate,lr,num_epochs,log_epochs,save_path="mpl_best_model.pdparams"):
        ...
        loss_fn=MultiCrossEntropyLoss(self.model)
        metric=accuracy
